...
public class LlmVotingService
{
    ...
    /// </summary>
    private static string ExtractJson(string text) => LegionJson.ExtractObject(text);

    /// <summary>
    /// Same as <see cref="ExtractJson"/> but for a top-level JSON array,
    /// returning <c>"[]"</c> on miss.
    /// </summary>
    private static string ExtractJsonArray(string text) => LegionJson.ExtractArray(text);
}
