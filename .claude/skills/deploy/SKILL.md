---
name: deploy
description: Render README.md, sync MindAttic.Components, and FTP-upload index.htm to mindattic.com/mindattic.legion/. Runs scripts/cli/deploy.bat.
---

When invoked:

1. Run `scripts\cli\deploy.bat` from the project root (`D:\Projects\MindAttic\MindAttic.Legion`).
2. The script will:
   ...
   - `sync-landing-page.ps1 -Subscriber MindAttic.Legion` -- splices OutfitFont / AtticFont / Cyberspace marker blocks into `index.htm`.
   - Stamp `<!-- Last Updated: ... -->` at the top of `index.htm`.
   - FTPS upload `index.htm` to `/mindattic.com/mindattic.legion/` (defined in `scripts/cli/deploy.settings.json`).
